...
abstract class Cart implements ActiveRecordInterface
{
    ...
    public function setProductId($v)
    {
        ...
        if ($this->product_id !== $v) {
            $this->product_id = $v;
            $this->modifiedColumns[CartTableMap::COL_PRODUCT_ID] = true;
        }
    ...
    public function setQuantity($v)
    {
        ...
        if ($this->quantity !== $v) {
            $this->quantity = $v;
            $this->modifiedColumns[CartTableMap::COL_QUANTITY] = true;
        }
    ...
    public function setTotalPrice($v)
    {
        ...
        if ($this->total_price !== $v) {
            $this->total_price = $v;
            $this->modifiedColumns[CartTableMap::COL_TOTAL_PRICE] = true;
        }
    ...
    public function setStatus($v)
    {
        ...
        if ($this->status !== $v) {
            $this->status = $v;
            $this->modifiedColumns[CartTableMap::COL_STATUS] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : CartTableMap::translateFieldName('Quantity', TableMap::TYPE_PHPNAME, $indexType)];
            $this->quantity = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : CartTableMap::translateFieldName('Price', TableMap::TYPE_PHPNAME, $indexType)];
            $this->price = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : CartTableMap::translateFieldName('TotalPrice', TableMap::TYPE_PHPNAME, $indexType)];
            $this->total_price = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : CartTableMap::translateFieldName('UserId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->user_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : CartTableMap::translateFieldName('Status', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CartTableMap::COL_QUANTITY)) {
            $modifiedColumns[':p' . $index++]  = 'quantity';
        }
        if ($this->isColumnModified(CartTableMap::COL_PRICE)) {
            $modifiedColumns[':p' . $index++]  = 'price';
        }
        if ($this->isColumnModified(CartTableMap::COL_TOTAL_PRICE)) {
            $modifiedColumns[':p' . $index++]  = 'total_price';
        }
        if ($this->isColumnModified(CartTableMap::COL_USER_ID)) {
            $modifiedColumns[':p' . $index++]  = 'user_id';
        }
        if ($this->isColumnModified(CartTableMap::COL_STATUS)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CartTableMap::COL_QUANTITY)) {
            $criteria->add(CartTableMap::COL_QUANTITY, $this->quantity);
        }
        if ($this->isColumnModified(CartTableMap::COL_PRICE)) {
            $criteria->add(CartTableMap::COL_PRICE, $this->price);
        }
        if ($this->isColumnModified(CartTableMap::COL_TOTAL_PRICE)) {
            $criteria->add(CartTableMap::COL_TOTAL_PRICE, $this->total_price);
        }
        if ($this->isColumnModified(CartTableMap::COL_USER_ID)) {
            $criteria->add(CartTableMap::COL_USER_ID, $this->user_id);
        }
        if ($this->isColumnModified(CartTableMap::COL_STATUS)) {
            $criteria->add(CartTableMap::COL_STATUS, $this->status);
        }

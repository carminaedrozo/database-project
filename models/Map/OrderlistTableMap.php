...
/**
 * This class defines the structure of the 'orderlist' table.
 *
 *
 *
 ...
 *
 */
class OrderlistTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.OrderlistTableMap';
     ...
     */
    const OM_CLASS = '\\Orderlist';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Orderlist';
    ...
    public function initialize()
    {
        ...
        $this->setPhpName('Orderlist');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\Orderlist');
        $this->setPackage('');
    ...
    {
        return $withPrefix ? OrderlistTableMap::CLASS_DEFAULT : OrderlistTableMap::OM_CLASS;
    }
     ...
     * @return array           (Orderlist object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = OrderlistTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = OrderlistTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + OrderlistTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = OrderlistTableMap::OM_CLASS;
            /** @var Orderlist $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            OrderlistTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = OrderlistTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = OrderlistTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(OrderlistTableMap::DATABASE_NAME)->getTable(OrderlistTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(OrderlistTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(OrderlistTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new OrderlistTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Orderlist or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or Orderlist object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Orderlist) { // it's a model object
        ...
        if ($values instanceof Criteria) {
            ...
            foreach ((array) $values as $singleval) {
                OrderlistTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return OrderlistQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Orderlist or Criteria object.
     *
     * @param mixed               $criteria Criteria or Orderlist object containing data that is used to create the INSERT statement.
...
//
OrderlistTableMap::buildTableMap();
